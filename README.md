...
**Last Updated:** July 2026 — Fixed cross-user personal_flag data leakage, removed all remaining Alert.alert usage, migrated Gemini to a server-side Edge Function, implemented legal pages, removed dead dependencies, corrected documentation drift against live repo
...
| Onboarding routing loop                        | `_layout.tsx` reads AsyncStorage directly (via `constants/onboarding.ts`) per segment change; `!inAuthGroup` guard added                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Dual`useOnboarding` hook instances           | Hook removed entirely;`_layout.tsx` and `onboarding.tsx` both call the plain async helpers in `constants/onboarding.ts` directly                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
│   └── onboarding.ts            ✅ AsyncStorage read/write helpers — no hook wrapper
│
├── lib/
│   ├── supabase.ts              ✅
...
├── supabase/
│   ├── functions/
│   │   └── analyze-ingredients/ ✅ Deno Edge Function — Gemini key lives here server-side
│   └── migrations/
...
> Corrected in v2.2: `hooks/useOnboarding.ts` and `hooks/useHistory.ts` don't exist in the live repo (onboarding uses the plain helpers above directly; history logic is inlined in `history.tsx`), and there is no `components/ui/` folder. This document previously listed all three.
